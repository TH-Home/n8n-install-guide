...
## **1.1 เลือกวิธีติดตั้ง (Deployment Method)**
...
## **1.2 สิ่งที่ต้องเตรียม (Prerequisites)**
...
2. ตั้งค่า:  
   * **Name:** \``` n8n-server` ``  
   * **Region:** เลือก *us-central1*, *us-west1* หรือ *us-east1* (เข้า Free Tier)  
   * **Zone:** เลือกใดก็ได้ใน Region นั้น  
   * **Series:** E2  
   * **Machine type:** e2-micro (Free Tier)  
      *(หมายเหตุ: ถ้าต้องการ RAM/CPU มากขึ้น เลือก e2-medium แต่จะมีค่าใช้จ่าย)*
...
```` ``` ````  
*(แก้ \``` n8n.thho.me` `` เป็นโดเมนจริงของคุณ)*  
...
3. เพิ่ม 2 Record นี้:  
   * **Root domain** → ให้ผู้ใช้เข้า \``` thho.me` ``  
     * Type: \``` A` ``  
     * Name: \``` @` ``  
     * IPv4 address: \``` 127.0.0.1` `` *(หรือเว้นไว้ถ้าใช้ Tunnel แบบเต็ม)*  
...
4. ปรับ \``` restore_backup` `` และ \``` restore_latest` `` ให้ restart n8n \+ maintenance\_tasks อัตโนมัติหลัง restore
